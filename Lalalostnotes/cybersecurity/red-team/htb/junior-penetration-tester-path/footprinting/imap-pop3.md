...
* **Akses Email di Server**
  * IMAP memungkinkan akses dan pengelolaan email langsung di server, berbeda dengan POP3 yang hanya mengunduh email.
  * Mendukung struktur folder dan sinkronisasi antar perangkat.
* **Mode Online dan Sinkronisasi**
  * Email tetap tersimpan di server hingga dihapus.
  * IMAP memungkinkan beberapa klien mengakses email secara bersamaan.
  * Beberapa klien menawarkan mode offline dengan sinkronisasi saat terhubung kembali.
* **Komunikasi dan Port**
  * Menggunakan **port 143** (default) dan **port 993** (SSL/TLS).
  * Menggunakan perintah berbasis teks dalam format ASCII.
  * Autentikasi dengan **username** dan **password** sebelum mengakses email.
* **Keunggulan IMAP**
  * Email yang dikirim bisa disalin ke folder IMAP, memungkinkan akses dari perangkat mana pun.
  * Mendukung pembuatan folder dan struktur penyimpanan yang lebih rapi.
* **Keamanan**
  * Secara default, IMAP mengirimkan data dalam **plaintext**.
  * Untuk keamanan, server biasanya menerapkan **SSL/TLS** untuk enkripsi komunikasi.
...
```shell-session
lalalostcode@htb[/htb]$ sudo nmap 10.129.14.128 -sV -p110,143,993,995 -sC
...
```shell-session
lalalostcode@htb[/htb]$ curl -k 'imaps://10.129.14.128' --user user:p4ssw0rd
...
```shell-session
lalalostcode@htb[/htb]$ curl -k 'imaps://10.129.14.128' --user cry0l1t3:1234 -v
...
```shell-session
lalalostcode@htb[/htb]$ openssl s_client -connect 10.129.14.128:pop3s
...
```shell-session
lalalostcode@htb[/htb]$ openssl s_client -connect 10.129.14.128:imaps
